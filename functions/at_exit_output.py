...
def at_exit_output() -> None:
    ...
    if tmp_path.is_dir():
        ...
        print(f"TMP DIR:                     DID NOT EXIST WHEN SCRIPT RAN - FIRST RUN?")
    
    if comp_path.is_dir():
        print(f"COMP FILE COUNT: {sum(1 for item in comp_path.iterdir() if item.is_file())}")
    else:
        print(f"COMP DIR:                    DID NOT EXIST WHEN SCRIPT RAN - FIRST RUN?")
    
    if log_path.is_dir():
        print(f"LOG FILE COUNT:  {sum(1 for item in log_path.iterdir() if item.is_file())}")
    else:
        print(f"LOG DIR:                     DID NOT EXIST WHEN SCRIPT RAN - FIRST RUN?")
