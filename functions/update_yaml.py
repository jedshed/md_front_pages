    ...
    try:
        for attempt in range(retries):
            try:
                ...
                frontmatter_yaml = re.sub(r'^author:.*', f'author: {meta_sitename}', frontmatter_yaml, flags=re.M)
...
# def update_yaml(latest_md_title, meta_url, meta_sitename, meta_filedate, meta_date, meta_author, do_func, do_display_frontmatter_yaml, retries=10,):

#     func_text = "UPDATE YAML FRONTMATTER"
#     print(f"{func_text:<{conf.fpad}} ...", end="\r", flush=True)

#     # Do Func Check
#     if not do_func:
#         print(f"{func_text:<{conf.fpad}} ... SKIPPED")
#         func.debug_delay_long()
#         return
...
#                 frontmatter_yaml = re.sub(r'^source:.*', f'source: {meta_url}', frontmatter_yaml, flags=re.M)
#                 frontmatter_yaml = re.sub(r'^author:.*', f'author: {meta_sitename}', frontmatter_yaml, flags=re.M)
#                 frontmatter_yaml = re.sub(r'^created:.*', f'created: {meta_filedate}', frontmatter_yaml, flags=re.M)
...
#                 else:
#                     raise  # Re-raise error if all retries fail
#         print(f"{func_text:<{conf.fpad}} ... DONE")
#     except Exception as e:
#         print(f"Failed to: {func_text:<{conf.fpad}} {e}")
    

#     display_frontmatter_yaml_text = "--- DISPLAY FRONTMATTER YAML"
#     print(f"{display_frontmatter_yaml_text:<{conf.fpad}} ...", end="\r", flush=True)

#     if not do_display_frontmatter_yaml:
#         print(f"{display_frontmatter_yaml_text:<{conf.fpad}} ... SKIPPED")
#         return frontmatter_yaml


#     if do_display_frontmatter_yaml:

#         # RePrint func_text to overide \r carriage return if DEBUG SKIPPED not invoked
#         print(f"{display_frontmatter_yaml_text:<{conf.fpad}} ...")
